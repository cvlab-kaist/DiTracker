...
class TapVidDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.dataset_type == "kinetics":
            ...
            if fast_eval:
                points_dataset = local_random.sample(points_dataset, 30)
            ...
            with open(data_root, "rb") as f:
                ...
                # self.points_dataset = {'goat': self.points_dataset['goat']}
...
class CorruptedTapVidDataset(TapVidDataset):
    """
    TAP-Vid dataset with ImageNet-C corruptions applied
    """
    def __init__(
        self,
        dataset_type,
    ...
    def __getitem__(self, index):
        ...
        if self.corruption_name is not None:
            # data.video shape: (T, C, H, W) as torch tensor
            video = data.video
            
            # Convert to numpy (T, H, W, C) for corruption
            video_np = video.permute(0, 2, 3, 1).cpu().numpy()
            video_np = video_np.astype(np.uint8)
            # print(video_np) 
            
            # Apply corruption to each frame
            corrupted_frames = []
            ...
            corrupted_video = np.stack(corrupted_frames)
            corrupted_video = torch.from_numpy(corrupted_video).float()
            corrupted_video = corrupted_video.permute(0, 3, 1, 2)
            
            data.video = corrupted_video
