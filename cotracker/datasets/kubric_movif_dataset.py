...
class CoTrackerDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if wi is not None and index % 200 == 0 and _ps is not None:
          print(f"[worker {wi.id}] idx={index} RSS={_ps.memory_info().rss/1e9:.2f} GB")
    ...
    def add_spatial_augs(self, rgbs, trajs, visibles, crop_size):
        ...
        # rgbs = [rgb.astype(np.) for rgb in rgbs]
...
class KubricMovifDataset(CoTrackerDataset):
    ...
    def _read_frame(self, path):
        # imageio returns uint8 already; keep it that way
        return imageio.v2.imread(path)
    
    def _sample_indices(self, num_frames, seq_len, frame_rate):
        usable = num_frames - (seq_len * frame_rate) + 1
        start = 0 if usable <= 0 else np.random.randint(0, usable)
        idxs = [start + t*frame_rate for t in range(seq_len)]
        return idxs
    ...
    def getitem_helper(self, index):
        ...
        if self.crop_size[0] > 512 or self.crop_size[1] > 512:
            if self.crop_size[0] > self.crop_size[1]:
                w = int(512 * (self.crop_size[1] / self.crop_size[0]))
                crop_size = (512, w)
            else:
                h = int(512 * (self.crop_size[0] / self.crop_size[1]))
                crop_size = (h, 512)
        else:
        ...
        idxs = self._sample_indices(num_frames, seq_len, frame_rate)
        rgbs = [ self._read_frame(os.path.join(rgb_path, img_paths[i])) for i in idxs ]
        rgbs = np.stack(rgbs)

        traj_2d = traj_2d[idxs]         # (seq_len, N, 2)
        visibility = visibility[idxs]   # (seq_len, N)
        ...
        if self.crop_size != crop_size:
            rgbs = F.interpolate(rgbs, size=self.crop_size, mode='bilinear')
            trajs[..., 1] *= self.crop_size[0] / crop_size[0]
            trajs[..., 0] *= self.crop_size[1] / crop_size[1]
