...
class Visualizer:
    ...
    def visualize(
        ...
        # resize video frames to H and W
        if self.resize_to:
            B, T, C, H, W = video.shape
            video = F.interpolate(video.reshape(-1, C, H, W), size=(self.resize_to[0], self.resize_to[1]), mode="bilinear", align_corners=False)
            video = video.reshape(B, T, C, self.resize_to[0], self.resize_to[1])
            
            # scale pred_tracks to H and W
            tracks = tracks / torch.tensor([W/self.resize_to[1], H/self.resize_to[0]]).to(tracks.device)
