...
class CoTrackerThreeBase(nn.Module):
    def __init__(
        ...
        head_hooks=[[2]],
        cost_softmax=True,
        resnet_fuse_mode="concat",
    ):
        super(CoTrackerThreeBase, self).__init__()
        
        assert resnet_fuse_mode in ["concat", "add", None], "ResNet fuse mode should be 'concat' or 'add' or None"
        ...
        self.cost_softmax = cost_softmax
        self.use_resnet = False if resnet_fuse_mode is None else True
        self.resnet_fuse_mode = resnet_fuse_mode

        self.latent_dim = latent_dim
        self.hidden_dim = 256
        self.input_dim = 1110
        
        self.fnet = BasicEncoder(input_dim=3, output_dim=latent_dim, stride=stride)
        ...
        input_dim = r ** 4
        if resnet_fuse_mode == "concat":
            input_dim *= 2
        self.corr_mlp = Mlp(in_features=input_dim, hidden_features=384, out_features=256)
