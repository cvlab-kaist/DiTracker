...
from cotracker.models.build_cotracker import build_dit_cotracker
...
def run_eval(cfg: DefaultConfig):
    """
    The function evaluates DiTracker on a specified benchmark dataset based on a provided configuration.
    ...
    Args:
        cfg (DefaultConfig): An instance of DefaultConfig class which includes:
            ...
            - checkpoint (str): The path to the DiTracker model's checkpoint.
    ...
    if cfg.visualize:
        visualizer = Visualizer(
            save_dir=os.path.join(cfg.exp_dir),
            fps=7,
    ...
    elif "itto" in cfg.dataset_name:
        from cotracker.datasets.itto_mose_dataset import MoseTrackDataLoader
        
        dataset_root = os.path.join(cfg.dataset_root, "itto")
        annotation_dir = os.path.join(dataset_root, "mose", "annotations")
        video_dir = os.path.join(dataset_root, "mose", "frames")
        video_id_list = os.listdir(video_dir)
        
        test_dataset = MoseTrackDataLoader(
