...
def gaussian_blur(x, severity=1):
    ...
    # Revised: due to deprecation of multichannel argument in skimage
...
def fog(x, severity=1):
    ...
    max_val = x.max()
    h, w = x.shape[:2]
    x += c[0] * plasma_fractal(wibbledecay=c[1])[:h, :w][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    # [Revised] frost를 이미지 크기에 맞게 resize
    from skimage.transform import resize
    x_array = np.array(x)
    h, w = x_array.shape[:2]
    if frost.shape[:2] != (h, w):
        frost = resize(frost, (h, w), preserve_range=True, anti_aliasing=True)
    return np.clip(c[0] * x_array + c[1] * frost, 0, 255)
...
def snow(x, severity=1):
    ...
    # [Revised] snow를 이미지 크기에 맞게 resize
    h, w = x.shape[:2]
